 ...
 */
//onst mainnet = `https://mainnet.infura.io/${process.env.INFURA_ACCESS_TOKEN}`
const testnet = `http://23.106.122.108:8545`;
...
const getCurrentGasPrices = async () => {
  ...
  console.log("\r\n");
  log(`Current XOT Gas Prices (in GWEI):`.cyan);
  console.log("\r\n");
...
const main = async () => {
   ...
   */
  let destinationBalanceWei = web3.eth
    .getBalance(process.env.DESTINATION_WALLET_ADDRESS)
    .toNumber();
  let destinationBalance = web3.fromWei(destinationBalanceWei, "ether");

  log(
    `Destination wallet balance is currently ${destinationBalance} XOT`.green
  );
  ...
  log(`Your wallet balance is currently ${myBalance} XOT`.green);
   ...
   * @type {Number}
   */
  let amountToSend = Math.floor(Math.random() * (100 - 1)) + 1;
  ...
  let details = {
    ...
    nonce: nonce,
    chainId: testmode ? 49145 : 1, // EIP 155 chainId - mainnet: 1, rinkeby: 4
  };
   ...
   */
  const url = `https://testnet.xota.io/tx/${transactionId}`;
  log(url.cyan);
